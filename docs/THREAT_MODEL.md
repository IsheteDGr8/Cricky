...
| **Spoofing:** guess a scorer code                           | Random 10-character codes (~10¹⁵ combinations), stored only as hashes, redemption rate-limited, per match and revocable.                  |
| **Tampering:** edit or delete past balls                    | Events are create-only. Only the last event can be removed (undo), and only by that match's scorers. Completed matches lock.              |
...
| **Information disclosure:** read scorer codes or roles      | `scorerCodes` has no read rule; `roles` readable only by the user themselves.                                                             |
